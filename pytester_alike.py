...
MAIN_MOUNT_DIR= Path(THIS_DIR) /"mount_dir"
...
def main():
    ...
    # find all "test_*.py" files in MAIN_MOUNT_DIR
    print("Finding all test files in the current directory")
    test_files = [os.path.join(MAIN_MOUNT_DIR, f) for f in os.listdir(MAIN_MOUNT_DIR) if f.startswith("test_") and f.endswith(".py")]
